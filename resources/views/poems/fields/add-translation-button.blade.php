...
@if($poem->is_translated && !$poem->originalPoem)
  ...
  <dd><a class="btn" href="{{ Auth::check() ? route('poems/create', ['translated_fake_id' => $poem->fake_id]) : route('login', ['ref' => route('poems/create', ['translated_fake_id' => $poem->fake_id], false)]) }}">
      @lang('poem.add original work')</a></dd><br>
@else
  <dt><a class="btn create add-translate-btn"
